...
OnItemClickListener, OnItemSelectedListener {
    ...
    new ChannelsTask(this, new ChannelClient(getCertificateProblemDialog())) {
      ...
      public void finished(final SvdrpEvent event) {
        if (event == SvdrpEvent.CACHE_HIT
          ...
          final ArrayList<Channel> channels = ChannelClient.getChannels();
  ...
  private boolean useCache() {
    ...
    final ArrayList<Epg> cachedChannel = EpgCache.CACHE.get(currentChannel.getId());

    if (cachedChannel == null) {
      return false;
    }

    final Date nextForceCache = EpgCache.NEXT_REFRESH.get(currentChannel.getId());
  ...
  private void startEpgQuery(final boolean force) {
    ...
    final EpgClient epgClient = new EpgClient(currentChannel, getCertificateProblemDialog());
  ...
  private ArrayList<Epg> getCache() {
    final ArrayList<Epg> arrayList = EpgCache.CACHE.get(currentChannel.getId());
    if (arrayList == null) {
      return EMPTY;
    }
    return arrayList;
